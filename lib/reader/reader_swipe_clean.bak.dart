...
// Paywall pieces (make sure these files exist)
...
// ========================= Page model ==============================
// ⬅️ Added "paywall" to keep the switch exhaustive.
enum _PageType { cover, title, frontText, chapterTitle, chapterBody, paywall }
...
class _ReaderSwipeCleanState extends State<ReaderSwipeClean> {
  ...
  Future<void> _buildAllPages() async {
    ...
    // Log incoming FM list
    debugPrint(
      'FM incoming for ${widget.title}: ' +
      widget.frontMatter.map((f) => f.assetPath.split('/').last).join(' -> ')
    );

    // Front matter (already ordered in _template.dart)
    debugPrint(
      'Reader FM order for ${widget.title}: ' +
      widget.frontMatter.map((f) => f.assetPath.split('/').last).join(' -> ')
    );
    ...
    for (final fm in widget.frontMatter) {
      try {
        ...
        debugPrint('FM missing: ${fm.assetPath} ($e)');
      }
    }
  ...
  String _inferKeyFromChaptersDir(String base) {
    final segs = base.split('/').where((s) => s.isNotEmpty).toList();
    return segs.isEmpty ? base : segs.last;
  }
  ...
  Future<void> _buildChaptersFromFolder(String base, int capacity) async {
    ...
    final key = _inferKeyFromChaptersDir(base);

    for (int n = 1; n <= total; n++) {
      // 🔒 paywall gate
      if (!AccessManager.instance.canAccessChapter(key, n)) {
        _pages.add(_Entry.paywall(key));
        break;
      }
  ...
  Future<void> _buildChaptersFromMonolithic(String manuscriptPath, int capacity) async {
    ...
    final key = _inferKeyFromChaptersDir(widget.chaptersDir ?? widget.title.toLowerCase().replaceAll(' ', ''));

    for (int i = 0; i < count; i++) {
      final chNum = i + 1;

      // 🔒 paywall gate
      if (!AccessManager.instance.canAccessChapter(key, chNum)) {
        _pages.add(_Entry.paywall(key));
        break;
      }
      ...
      final titleLine = (headMatch?.group(0) ?? 'Chapter $chNum').trim();
                ...
                children: [
                  ...
                  PageView.builder(
                    ...
                    onPageChanged: (i) {
                      ...
                      final key = _inferKeyFromChaptersDir(widget.chaptersDir ?? widget.title);
                      final ch = _currentChapterOr1();
                      AccessManager.instance.saveProgress(key, chapter: ch, page: i);
                    },
                  ...
                  // Dropdown banner (Back + Chapters) — shows on tap, hides after 10s
...
class _Entry {
  ...
  factory _Entry.chapterBody(String t) => _Entry._(_PageType.chapterBody, text: t);
  factory _Entry.paywall(String bookKey) => _Entry._(_PageType.paywall, text: bookKey);
}
...
class _PageCard extends StatelessWidget {
  ...
  switch (entry.type) {
    ...
    // ✅ This is the one you’re missing or was malformed:
    case _PageType.paywall:
      return PaywallScreen(bookKey: entry.text ?? '');
  }
}
